...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FReplicEditorCallEventRenameValidationTest,
	"Replic.Editor.CallEventRenameValidation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FReplicEditorCallEventRenameValidationTest::RunTest(const FString& Parameters)
{
	ConfigureReplicEditorAutomationMetadata();

	UBlueprint* Blueprint = CreateTransientReplicBlueprint(TEXT("ReplicEditorCallEventRenameValidation"));
	UEdGraph* Graph = GetEventGraph(Blueprint);
	UK2Node_CustomEvent* CustomEventNode = Graph ? AddNodeToGraph<UK2Node_CustomEvent>(Graph) : nullptr;
	UK2Node_ReplicCallEvent* CallEventNode = Graph ? AddNodeToGraph<UK2Node_ReplicCallEvent>(Graph) : nullptr;
	if (!TestNotNull(TEXT("Transient test blueprint should be created"), Blueprint)
		|| !TestNotNull(TEXT("Transient test event graph should exist"), Graph)
		|| !TestNotNull(TEXT("Custom event node should be created"), CustomEventNode)
		|| !TestNotNull(TEXT("Replic call event node should be created"), CallEventNode))
	{
	...
	CustomEventNode->CustomFunctionName = FName(TEXT("ReplicEventBeforeRename"));
	CustomEventNode->AllocateDefaultPins();
	CustomEventNode->GetUserDefinedMetaData().SetMetaData(ReplicMetadata::EventEnabled, FString(TEXT("true")));
	CustomEventNode->GetUserDefinedMetaData().SetMetaData(ReplicMetadata::EventPermissionMode, FString(TEXT("OwnerOnly")));
	CustomEventNode->GetUserDefinedMetaData().SetMetaData(ReplicMetadata::EventMode, FString(TEXT("ReplicateAll")));
	...
	CustomEventNode->CustomFunctionName = FName(TEXT("ReplicEventAfterRename"));
	TArray<TSharedPtr<FReplicPinOptionItem>> RenamedEventOptions;
	ReplicPinOptionResolver::BuildOptions(CallEventNode->GetEventNamePin(), RenamedEventOptions);
	TestTrue(TEXT("Event dropdown should contain the current Replic event name after rename"), OptionsContain(RenamedEventOptions, CustomEventNode->CustomFunctionName));
	TestFalse(TEXT("Event dropdown should not keep the stale generated event name after rename"), OptionsContain(RenamedEventOptions, FName(TEXT("ReplicEventBeforeRename"))));

	FCompilerResultsLog ValidationLog;
	ValidationLog.bSilentMode = true;
	CallEventNode->EarlyValidation(ValidationLog);

	TestTrue(TEXT("A stale Replic event name should produce a validation error immediately after rename"), ValidationLog.NumErrors > 0);
	return ValidationLog.NumErrors > 0;
}
